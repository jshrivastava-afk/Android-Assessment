...
class MainActivity : AppCompatActivity(), EditStudentIntentListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel = ViewModelProvider(this).get(View_StudentsModel::class.java)
        (viewModel as View_StudentsModel).allStudents.observe(this, { students -> adapter.setStudents(students) })
        ItemTouchHelper(object : ItemTouchHelper.SimpleCallback(0, ItemTouchHelper.LEFT or ItemTouchHelper.RIGHT) {
            ...
            override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int) {
                (viewModel as View_StudentsModel).delete(adapter.getStudentAt(viewHolder.adapterPosition))
                Toast.makeText(this@MainActivity, "Student deleted", Toast.LENGTH_SHORT).show()
            }
        }).attachToRecyclerView(recyclerView)
        button.setOnClickListener(View.OnClickListener {
            val intent = Intent(this@MainActivity, Edit_StudentActivity::class.java)
